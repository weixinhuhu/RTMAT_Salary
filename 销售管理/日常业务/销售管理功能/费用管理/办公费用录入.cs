...
using 销售管理.DAL.DataSetOfficeExpensesTableAdapters;
using System.Data;

namespace 销售管理.日常业务
{
    public partial class 办公费用录入 : Form
    {
        public 办公费用录入()
        {
            InitializeComponent();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                toolTip1.Show("请输入费用分摊日期！", dtpDate1, 0, dtpDate1.Height, 2000);
                return;
            }
            ...
            if (mId == -1)
            {
                ret = new T_OfficeExpensesTableAdapter().Insert1((long)cmbUserName.SelectedValue, Convert.ToDecimal(Txt1.Text), Convert.ToDecimal(Txt2.Text), Convert.ToDecimal(Txt3.Text), Convert.ToDecimal(Txt4.Text), Convert.ToDecimal(Txt5.Text), Convert.ToDecimal(TxtSumMoney.Text), dtpDate1.Value.Date);
            }
            else
            {
                ret = new T_OfficeExpensesTableAdapter().UpdateById((long)cmbUserName.SelectedValue, Convert.ToDecimal(Txt1.Text), Convert.ToDecimal(Txt2.Text), Convert.ToDecimal(Txt3.Text), Convert.ToDecimal(Txt4.Text), Convert.ToDecimal(Txt5.Text), Convert.ToDecimal(TxtSumMoney.Text), dtpDate1.Value.Date, mId);
            }
        ...
        {    
            ...
            else
            {
                ...
                dt = new T_OfficeExpensesTableAdapter().GetDataById(mId);

                if (dt.Rows.Count > 0)
                {
                    var mRow = (销售管理.DAL.DataSetOfficeExpenses.T_OfficeExpensesRow)dt.Rows[0];
